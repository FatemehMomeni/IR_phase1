...
class BSBIIndex:
    ...
    def parse_block(self, block_dir_relative):
        ...
        """
        text = dict()
        for file in sorted(next(os.walk(block_dir_relative))[2]):
            with open(file, 'r', encoding='utf-8-sig') as f:
                text[f] = f.read()
            f.close()

        s_file = open('stopWords.txt', 'r', encoding='utf-8-sig')
        stops = s_file.read()
        s_file.close()
        ...
        for t in text:
            text[t] = normalizer.normalize(text[t])
            text[t] = text[t].replce('\u200c', ' ')
            text[t] = text[t].replce('amp', '')
            text[t] = text[t].replce('nbsp', '')

        vectorizer = CountVectorizer(encoding='utf-8-sig', stop_words=stops)
        for t in text:
            dt_matrix = vectorizer.fit_transform(text[t])
            text[t] = vectorizer.get_feature_names()

        terms = dict()
        for t in text:
            if text[t][0] not in digits:
                terms[t] = text[t]

        td_pairs = list(tuple())
        for t in terms:
            d_id = self.doc_id_map.__getitem__(t)
            for i in terms[t]:
                t_id = self.term_id_map.__getitem__(terms[t][i])
                td_pairs.append((d_id, t_id))
